...
function HeroSection({ onNav }) {
  ...
  return (
    <section style={{ paddingTop: isMobile ? 120 : 160, paddingBottom: isMobile ? 40 : 60, position: 'relative', overflow: 'hidden', minHeight: isMobile ? 'auto' : '100vh' }} data-label="Hero">
      <ParticleField count={isMobile ? 36 : 70} />
...
// =========================================================
const TESTIMONIALS = [
  { quote: 'They built our AI feature in 6 weeks — including the parts we didn\'t know we needed. Outpaced our in-house team.', name: 'Maya Hernández', role: 'VP Engineering · Kairos Labs', color: '#f43f5e' },
          ...
          <Marquee key={i} speed={28 + i * 4} reverse={i % 2 === 1}>
            {row.map((t, j) => (
              <div key={j} style={{
                ...
                background: j % 3 === 0 ? 'rgba(244,63,94,0.04)' : 'transparent',
              }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }} className="pricing-grid">
          {plans.map((p, i) => (
            <div key={p.name} className="card reveal" data-delay={i + 1} style={{
              ...
              background: p.accent ? 'rgba(244, 63, 94, 0.04)' : 'var(--surface)',
              position: 'relative',
              minHeight: 480,
            }}>
              {p.accent && (
