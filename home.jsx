...
function HeroSection({ onNav }) {
  const [vw, setVw] = hUseState(typeof window !== 'undefined' ? window.innerWidth : 1280);
  hUseEffect(() => {
  ...
  const globeSize = isSmall ? 280 : isMobile ? 380 : 640;
  const gapBetween = isSmall ? 200 : isMobile ? 280 : 420;

  return (
    <section style={{ paddingTop: isMobile ? 120 : 160, paddingBottom: isMobile ? 40 : 60, position: 'relative', overflow: 'hidden', minHeight: isMobile ? 'auto' : '100vh' }} data-label="Hero">
      <FlowingPaths count={isMobile ? 22 : 38} />
      <DotGridBackground opacity={0.4} />

      {/* Top status bar */}
      <div className="container reveal" style={{ marginBottom: isMobile ? 48 : 80 }}>
        <div style={{
          display: 'flex',
          ...
          <div className="eyebrow">[001] INTELLIGENCE, ENGINEERED</div>
          <div style={{ display: 'flex', gap: isMobile ? 12 : 24, flexWrap: 'wrap' }}>
            <div className="mono text-muted" style={{ fontSize: 11, letterSpacing: '0.12em' }}>
              <span className="text-accent">●</span> EST. 2012
            </div>
            <div className="mono text-muted hide-mobile" style={{ fontSize: 11, letterSpacing: '0.12em' }}>
              <span style={{ width: 6, height: 6, borderRadius: 50, background: 'var(--accent)', display: 'inline-block', marginRight: 8 }} />
              +971 50 119 9879
            </div>
          </div>
        </div>
      ...
      <div className="container" style={{ position: 'relative', display: 'grid', gridTemplateColumns: '1fr', gap: 0 }}>
        ...
        {/* Globe centered — with parallax (monochrome) */}
        <HeroParallax speed={0.18}>
          <div style={{
            position: 'absolute',
            left: '50%',
            top: '52%',
        ...
        {/* Headline — interleaved with globe, with split-word stagger */}
        <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', paddingTop: isMobile ? 10 : 30 }}>
          <h1 className="h-display" style={{ marginBottom: 0 }}>
            <SplitText text="We build" />
            <span> </span>
            <span className="italic-serif" style={{ color: 'var(--accent)' }}>
              <SplitText text="intelligent" delay={0.2} />
            </span>
          </h1>
          <div style={{ height: gapBetween }} />
          <h1 className="h-display" style={{ marginTop: 0 }}>
            <SplitText text="software & systems." delay={0.4} />
          </h1>
        </div>

        {/* Subheading + CTAs */}
        <div className="reveal hero-sub" data-delay="3" style={{
          ...
          <p style={{ fontSize: 'clamp(15px, 1.4vw, 22px)', color: 'var(--text-dim)', maxWidth: 540, margin: isMobile ? '0 auto' : 0, lineHeight: 1.5 }}>
        ...
        <div className="reveal hero-metrics" data-delay="4" style={{
          ...
          {[
            ...
            <div key={i} style={{
              ...
              gap: 6,
              borderLeft: !isMobile && i > 0 ? '1px solid var(--border)' : 'none',
              paddingLeft: !isMobile && i > 0 ? 24 : 0,
            }}>
              <div style={{ fontFamily: 'var(--display)', fontSize: 'clamp(26px, 3.4vw, 44px)', fontWeight: 500, letterSpacing: '-0.03em' }}>
                <Counter to={m.v} suffix={m.suffix} decimals={m.decimals || 0} />
              </div>
              <div className="mono text-muted" style={{ fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase' }}>{m.label}</div>
            </div>
          ))}
        </div>
      </div>

      {!isMobile && <ScrollCue label="Scroll" />}
    </section>
  );
}
...
function ServiceCard({ s, large = false, delay }) {
...
function ServicesSection({ onNav }) {
  ...
  return (
    <section ref={sectionRef} className="section-light" style={{ position: 'relative', overflow: 'hidden' }}>
      <SectionNumberBackdrop number="02" side="right" sectionRef={sectionRef} />
      <FloatingTechObjects count={5} seed={2} />
      ...
      <div className="container">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }} className="services-grid">
          <ServiceCard s={SERVICES_LGST[0]} large />
          <ServiceCard s={SERVICES_LGST[1]} />
          <ServiceCard s={SERVICES_LGST[2]} />
          <ServiceCard s={SERVICES_LGST[3]} large />
          <ServiceCard s={SERVICES_LGST[4]} />
          <ServiceCard s={SERVICES_LGST[5]} />
          <ServiceCard s={SERVICES_LGST[6]} large />
        </div>
      </div>
      <style>{`
...
function AboutSection({ onNav }) {
  const sectionRef = hUseRef(null);
  return (
    <section ref={sectionRef} style={{ background: 'var(--bg-2)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', position: 'relative', overflow: 'hidden' }}>
      <SectionNumberBackdrop number="03" side="left" sectionRef={sectionRef} />
      <FloatingTechObjects count={4} seed={3} />
      {/* Rotating ring decoration */}
      <div style={{ position: 'absolute', right: -80, top: -80, opacity: 0.5, pointerEvents: 'none', zIndex: 0 }}>
        <RotatingRings size={300} />
      </div>
      <div className="container">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'start' }} className="about-grid">
          <div>
            ...
            <p className="reveal text-dim" data-delay="3" style={{ fontSize: 18, lineHeight: 1.6, marginBottom: 36, maxWidth: 540 }}>
              Founded in 2012, working with startups and Fortune 500s on the same standard: <span style={{ color: 'var(--text)' }}>real systems, measured impact, no theatre.</span>
            </p>
          ...
          <div className="reveal" data-delay="2" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div className="card" style={{ padding: 36, position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <div style={{ width: 44, height: 44, borderRadius: 50, background: 'linear-gradient(135deg, var(--accent), #ff7787)' }} />
                <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div className="card" style={{ padding: 28 }}>
                ...
                <div style={{ fontSize: 56, fontWeight: 500, letterSpacing: '-0.03em' }}>'12</div>
                <div className="text-muted mono" style={{ fontSize: 11, letterSpacing: '0.1em' }}>14 YEARS, 142 SHIPS</div>
              </div>
              <div className="card" style={{ padding: 28 }}>
...
function ProcessSection() {
  ...
  return (
    <section ref={sectionRef} className="section-light" style={{ position: 'relative', overflow: 'hidden' }}>
      <SectionNumberBackdrop number="04" side="right" sectionRef={sectionRef} />
      <FloatingTechObjects count={4} seed={4} />
      <GridBackdrop opacity={0.04} />
      <div className="container">
        <SectionHeader
      ...
      <style>{`
        ...
        @media (max-width: 900px) {
          ...
          .process-grid > * { border-right: 0 !important; padding-left: 0 !important; padding-right: 16px !important; }
          .process-grid > *:nth-child(2n) { padding-left: 16px !important; padding-right: 0 !important; }
        }
        @media (max-width: 540px) {
          .process-grid { grid-template-columns: 1fr !important; }
          .process-grid > * { padding: 32px 0 !important; border-top: 1px solid var(--border) !important; }
          .process-grid > *:first-child { border-top: 0 !important; }
        }
      `}</style>
    </section>
  );
...
function CaseStudiesSection({ onNav, onOpenCase }) {
  ...
  return (
    <section ref={sectionRef} style={{ background: 'var(--bg-2)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', position: 'relative', overflow: 'hidden' }}>
      <SectionNumberBackdrop number="05" side="left" sectionRef={sectionRef} />
      <FloatingTechObjects count={4} seed={5} />
      <div className="container">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, marginBottom: 64, alignItems: 'end' }} className="services-head">
          <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {CASES_LGST.map((c, i) => (
            <CaseRow key={c.id} c={c} index={i} onClick={() => onOpenCase(c)} last={i === CASES_LGST.length - 1} />
          ))}
        </div>
      </div>
    </section>
    ...
    >
      <style>{`
        @media (max-width: 768px) {
          .case-row { grid-template-columns: 36px 1fr 44px !important; gap: 16px !important; padding: 24px 4px !important; }
          .case-row .case-hide-m { display: none !important; }
          .case-row .case-arrow > div { width: 44px !important; height: 44px !important; }
        }
      `}</style>
      ...
      <div className="mono case-hide-m" style={{ fontSize: 12, letterSpacing: '0.1em', color: 'var(--text-dim)', textTransform: 'uppercase' }}>{c.discipline}</div>
      <div className="mono case-hide-m" style={{ fontSize: 12, letterSpacing: '0.1em', color: 'var(--text-dim)' }}>{c.year}</div>
      <div className="case-arrow" style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <div style={{
...
function StatsSection() {
  const sectionRef = hUseRef(null);
  return (
    <section ref={sectionRef} style={{ padding: '180px 0', position: 'relative', overflow: 'hidden' }}>
      <SectionNumberBackdrop number="06" side="right" sectionRef={sectionRef} />
      <FloatingTechObjects count={6} seed={6} />
      <GridBackdrop opacity={0.05} />
      <SectionParallax sectionRef={sectionRef} speed={80}>
...
// =========================================================
const TESTIMONIALS = [
  { quote: 'They built our AI feature in 6 weeks — including the parts we didn\'t know we needed. Outpaced our in-house team.', name: 'Maya Hernández', role: 'VP Engineering · Kairos Labs', color: '#ffffff' },
  { quote: 'Most agencies sell hours. Nesus sells outcomes. We hit our launch window because they actually owned it.', name: 'Tomás Aguilar', role: 'CEO · Arcform', color: '#a0a0a0' },
  { quote: 'Our compliance team approved their SOC2 documentation faster than anything I have signed off on in 12 years.', name: 'Dr. Renu Iyer', role: 'CTO · Helix-9', color: '#999999' },
  { quote: 'A rare studio that can do design, infra, and AI evals in the same standup. The whole thing felt like one team.', name: 'James Okafor', role: 'Head of Product · Quantly', color: '#d4d4d4' },
];
...
function TestimonialsSection() {
  ...
  return (
    <section ref={sectionRef} style={{ background: 'var(--bg-2)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', position: 'relative', overflow: 'hidden' }}>
      <SectionNumberBackdrop number="07" side="right" sectionRef={sectionRef} />
      <FloatingTechObjects count={3} seed={7} />
      <div style={{ position: 'absolute', left: -100, bottom: -100, opacity: 0.4, pointerEvents: 'none', zIndex: 0 }}>
        <RotatingRings size={260} />
      </div>
      <div className="container">
...
function TechStackSection() {
  ...
  return (
    <section ref={sectionRef} className="section-light" style={{ position: 'relative', overflow: 'hidden' }}>
      <SectionNumberBackdrop number="08" side="left" sectionRef={sectionRef} />
      <GridBackdrop opacity={0.04} />
          ...
          <Marquee key={i} speed={28 + i * 4} reverse={i % 2 === 1}>
            {row.map((t, j) => (
              <div key={j} style={{
                ...
                background: j % 3 === 0 ? 'rgba(255, 255, 255, 0.04)' : 'transparent',
              }}>
...
function PricingSection({ onNav }) {
  ...
  return (
    <section ref={sectionRef} style={{ position: 'relative', overflow: 'hidden' }}>
      <SectionNumberBackdrop number="09" side="right" sectionRef={sectionRef} />
      <FloatingTechObjects count={5} seed={9} />
      <div className="container">
        <SectionHeader
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }} className="pricing-grid">
          {plans.map((p, i) => (
            <div key={p.name} className="card reveal" data-delay={i + 1} style={{
              ...
              background: p.accent ? 'rgba(255, 255, 255, 0.04)' : 'var(--surface)',
              position: 'relative',
              minHeight: 480,
            }}>
              {p.accent && (
      ...
      <style>{`
        ...
        @media (max-width: 540px) {
          .pricing-grid > div { padding: 28px !important; min-height: auto !important; }
        }
      `}</style>
    </section>
  );
...
function FAQSection() {
  ...
  return (
    <section ref={sectionRef} className="section-light" style={{ position: 'relative', overflow: 'hidden' }}>
      <SectionNumberBackdrop number="10" side="left" sectionRef={sectionRef} />
      <GridBackdrop opacity={0.04} />
      <div className="container">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.6fr', gap: 80, alignItems: 'start' }} className="faq-grid">
          <div style={{ position: 'sticky', top: 120 }}>
            ...
            <p className="reveal text-dim" data-delay="2" style={{ fontSize: 16, maxWidth: 400 }}>
              Not finding what you need? Most questions get a same-day answer over email — try <span style={{ color: 'var(--accent)' }}>nesus.info@nesusai.com</span>.
            </p>
          </div>
          <div>
            {FAQS.map((f, i) => (
...
function CTASection({ onNav }) {
  ...
  const progress = useElementProgress(ctaRef);
  return (
    <section ref={ctaRef} style={{ padding: '160px 0 120px', position: 'relative', overflow: 'hidden' }}>
      <FloatingOrb x="20%" y="30%" size={500} delay={1} />
      <FloatingTechObjects count={7} seed={11} />
      <GridBackdrop opacity={0.05} />
      ...
      <div className="container" style={{ position: 'relative', textAlign: 'center' }}>
        ...
        <h2 className="reveal" data-delay="1" style={{
          ...
          lineHeight: 0.95,
          marginBottom: 48,
          // subtle parallax — text rises as section enters
        ...
        <div className="reveal" data-delay="2" style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 60 }}>
          ...
          <a href="mailto:nesus.info@nesusai.com" className="btn btn-ghost" style={{ padding: '18px 28px', fontSize: 13 }}>
...
function Home({ onNav, onOpenCase }) {
  ...
  return (
    <main className="page-enter" data-screen-label="01 Home">
      ...
      <ClientsMarquee />
      <TechMarquee
        words={['AI Agents', 'RAG Pipelines', 'Multi-Modal', 'Fine-Tuning', 'Vector DBs', 'Edge Inference', 'Real-Time ML', 'Custom Eval Suites', 'Model Distillation']}
        speed={45}
        large
      />
      <ServicesSection onNav={onNav} />
      <TechMarquee
        words={['TypeScript', 'Python', 'Rust', 'Go', 'Swift', 'Kotlin', 'OpenAI', 'Anthropic', 'PyTorch', 'PostgreSQL', 'Snowflake', 'Kubernetes']}
        speed={60}
        reverse
      />
      ...
      <ProcessSection />
      <TechMarquee
        words={['Discover', 'Architect', 'Build', 'Ship', 'Scale', 'Measure', 'Iterate']}
        speed={42}
        large
        separator="●"
      />
